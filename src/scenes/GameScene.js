...
export default class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    platform(500,650,1000); platform(1250,610,360); platform(1660,545,330); platform(2050,630,360); platform(2500,555,420); platform(3010,620,450); platform(3540,530,400); platform(4020,640,360);
    this.player=this.physics.add.sprite(180,520,'rumi').setCollideWorldBounds(false); this.player.body.setSize(38,84); this.physics.add.collider(this.player,this.platforms);
    ...
    this.cameras.main.setBounds(0,0,GAMEPLAY.worldWidth,720); this.cameras.main.startFollow(this.player,true,.09,.09,260,50);
    this.notice=this.add.text(640,40,'Move: A/D or ←/→   Jump: W/Space   Dodge: K   Attack: placeholder',{fontFamily:'system-ui',fontSize:'17px',color:'#fff'}).setOrigin(.5).setScrollFactor(0).setDepth(110);
  }
  update(time){
    if(this.player.y>820){this.player.setPosition(Math.max(120,this.player.x-120),450);this.player.setVelocity(0);}
