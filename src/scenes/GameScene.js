...
export default class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.player=this.physics.add.sprite(180,430,this.hunter.texture).setCollideWorldBounds(false);this.player.body.setSize(Math.max(34,this.player.width-4),Math.max(72,this.player.height-4));this.physics.add.collider(this.player,this.platforms);
  ...
  update(time){
    ...
    const jump=Phaser.Input.Keyboard.JustDown(this.cursors.up)||Phaser.Input.Keyboard.JustDown(this.keys.W)||Phaser.Input.Keyboard.JustDown(this.keys.SPACE)||this.touch.consume('jump');if(jump&&grounded)this.player.setVelocityY(-GAMEPLAY.jumpSpeed);
    const dodge=Phaser.Input.Keyboard.JustDown(this.keys.K)||this.touch.consume('dodge');if(dodge&&time-this.lastDodge>=GAMEPLAY.dodgeCooldown){this.lastDodge=time;this.isAttacking=false;this.player.setVelocityX(this.lastFacing*this.hunter.dodgeSpeed);this.player.setAlpha(.5);this.time.delayedCall(GAMEPLAY.dodgeDuration,()=>this.player.setAlpha(1));}
  ...
  supportAttack(){
    ...
    const support=GAMEPLAY.hunters[supportId];
    this.flashSyncLabel(`${support.name} SUPPORT!`);
    const ghost=this.add.image(this.player.x-this.lastFacing*62,this.player.y,support.texture).setDepth(24).setAlpha(.92);
    this.tweens.add({targets:ghost,x:ghost.x+this.lastFacing*130,duration:210,ease:'Power2',onComplete:()=>this.tweens.add({targets:ghost,alpha:0,duration:160,onComplete:()=>ghost.destroy()})});
    const reach=supportId==='mira'?230:supportId==='zoey'?170:155;
    const damage=supportId==='mira'?34:supportId==='zoey'?27:30;
    ...
    this.tweens.add({targets:fx,alpha:0,scaleX:1.15,duration:280,onComplete:()=>fx.destroy()});
  ...
  duoCombo(){
    const ids=this.otherHunters(),supportId=ids[this.supportIndex%ids.length];this.supportIndex++;
    const support=GAMEPLAY.hunters[supportId];
    this.flashSyncLabel(`${this.hunter.name} + ${support.name}  DUO COMBO!`);
    const ally=this.add.image(this.player.x-this.lastFacing*75,this.player.y,support.texture).setDepth(24).setAlpha(.95);
    this.tweens.add({targets:ally,x:ally.x+this.lastFacing*180,duration:260,ease:'Power2'});
    ...
    this.tweens.add({targets:ring,scale:2.4,alpha:0,duration:420,onComplete:()=>ring.destroy()});
    this.damageEnemiesNear(this.player.x+this.lastFacing*120,this.player.y,260,125,58,2);
    this.cameras.main.shake(150,.005);
    this.time.delayedCall(380,()=>this.tweens.add({targets:ally,alpha:0,duration:150,onComplete:()=>ally.destroy()}));
  }
  ...
  fullSync(){
    ...
    ids.forEach((id,i)=>{
      ...
      const img=this.add.image(this.player.x+offsets[i],this.player.y-(i===1?8:0),h.texture).setDepth(27).setAlpha(.98);
      echoes.push(img);
      ...
      this.tweens.add({targets:beam,scaleX:1.55,alpha:0,duration:420+i*50,onComplete:()=>beam.destroy()});
    });
    const pulse=this.add.circle(this.player.x,this.player.y,80,0xffffff,.08).setDepth(24).setStrokeStyle(9,0xffa5f0,.9);
    this.tweens.add({targets:pulse,scale:4.4,alpha:0,duration:560,onComplete:()=>pulse.destroy()});
    ...
    this.cameras.main.shake(320,.009);
    this.time.delayedCall(500,()=>echoes.forEach(img=>this.tweens.add({targets:img,alpha:0,duration:180,onComplete:()=>img.destroy()})));
  }
  ...
  flashSyncLabel(text){
    this.comboText.setText(text).setScale(1.08);
    this.tweens.add({targets:this.comboText,scale:1,duration:180});
    this.time.delayedCall(900,()=>{if(this.comboText.text===text)this.comboText.setText('');});
  }
  ...
  hitEnemy(e,damage,step,buildSync=false){
    ...
    if(buildSync)this.gainSync(step===2?16:10);
  ...
  resetPlayer(){this.playerHealth=this.hunter.health;this.player.setPosition(180,430);this.player.setVelocity(0);this.player.clearTint();this.sync=0;this.comboText.setText('RECOVER!');this.time.delayedCall(800,()=>this.comboText.setText(''));}
  updateHUD(){
