...
export default class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.add.circle(950,125,65,0xe7dfff,.85).setScrollFactor(.12);this.add.text(620,75,'SEOUL // HUNTER COMBAT TEST',{fontFamily:'system-ui',fontSize:'22px',color:'#bba7ff'}).setScrollFactor(.35);
  ...
  performAttack(time){
    ...
    if(this.characterId==='zoey'&&step===2)this.fireZoeyProjectile(damage+8);
    else this.doMeleeAttack(reach,damage,step,duration);
  ...
  doMeleeAttack(reach,damage,step,duration){
    const hitX=this.player.x+this.lastFacing*(42+reach/2);
    const height=this.characterId==='mira'?48:68;
    ...
    this.enemies.getChildren().forEach(e=>{if(e.dead)return;const dx=e.x-this.player.x,inFront=Math.sign(dx)===this.lastFacing||Math.abs(dx)<30;if(inFront&&Math.abs(dx)<=reach+48&&Math.abs(e.y-this.player.y)<85)this.hitEnemy(e,damage,step);});
  }
  fireZoeyProjectile(damage){
    ...
    const overlap=this.physics.add.overlap(blade,this.enemies,(b,e)=>{if(b.hit||e.dead)return;b.hit=true;this.hitEnemy(e,damage,2);overlap.destroy();b.destroy();});
  ...
  resetPlayer(){this.playerHealth=this.hunter.health;this.player.setPosition(180,430);this.player.setVelocity(0);this.player.clearTint();this.comboText.setText('RECOVER!');this.time.delayedCall(800,()=>this.comboText.setText(''));}
  updateHUD(){this.healthBar.width=266*(this.playerHealth/this.hunter.health);this.healthText.setText(`${this.hunter.name}  ${this.playerHealth} / ${this.hunter.health}`);}
}
