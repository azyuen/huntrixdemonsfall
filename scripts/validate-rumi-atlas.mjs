...
const corruptStarts=[...new Set(invalid.filter(([i])=>i===0 || !invalid.some(([j])=>j===i-1)).map(([i])=>i))];
const snippets=corruptStarts.map(i=>[i,rumiAnimationBase64.slice(i,i+23)]);

const bytes = Buffer.from(rumiAnimationBase64, 'base64');
const sha = crypto.createHash('sha256').update(bytes).digest('hex');
console.log('Rumi atlas diagnostics',JSON.stringify({base64Length:rumiAnimationBase64.length,byteLength:bytes.length,sha,snippets,invalid:invalid.slice(0,40),padding:padding.slice(0,20),paddingCount:padding.length}));
...
if (padding.length && (padding.length>2 || padding[0] < EXPECTED_BASE64_LENGTH-2)) throw new Error(`Rumi atlas contains internal base64 padding at ${padding.join(',')}`);
